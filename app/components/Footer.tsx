...
export default function Footer() {
  ...
  return (
    <footer ref={footerRef} id="footer" className="relative bg-[#060606] overflow-hidden">
      {/* ── Info row ── */}
      <div className="relative z-10 px-[var(--page-padding)] pt-20 pb-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-10 md:gap-20 items-start">
            ...
            <nav className="flex flex-col gap-3">
              {footerLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="text-white/45 text-sm font-medium hover:text-white transition-colors duration-300"
                >
                  {link.label}
                </a>
              ))}
            ...
            <div className="flex flex-col items-start md:items-end gap-5">
              <div className="flex gap-3">
                {socialLinks.map((s) => (
                  <a
                    ...
                    aria-label={s.label}
                    className="w-11 h-11 rounded-lg border border-white/10 flex items-center justify-center text-white/35 hover:text-tiffany hover:border-tiffany/30 transition-all duration-300"
                  >
                    {s.icon}
                  </a>
                ))}
              ...
              <p className="text-white/15 text-[0.65rem] tracking-[0.05em]">
                &copy; {new Date().getFullYear()} eDrive. All rights reserved.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ── Giant Brand Text with car image ── */}
      <div className="relative h-[40vh] md:h-[50vh] lg:h-[55vh] overflow-hidden">
        ...
        <motion.div className="absolute inset-0" style={{ y: imageY }}>
          <Image
            ...
            alt="eDrive JetCar"
            fill
            className="object-cover object-center scale-110 opacity-[0.1]"
          />
        </motion.div>
